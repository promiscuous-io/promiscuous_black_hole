...
class TestCluster
  ...
  def create_test_topic
    sh "#{KAFKA_TOPIC} --zookeeper localhost:#{ZOOKP_PORT} --create --topic #{TOPIC} --partitions 2 --replication-factor 1"
    sh "#{KAFKA_TOPIC} --zookeeper localhost:#{ZOOKP_PORT} --create --topic #{TOPIC}.sync --partitions 2 --replication-factor 1"
  end
